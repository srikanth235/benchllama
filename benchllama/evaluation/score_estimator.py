...
from ..constants import PROMPT_EVAL_DURATION, PROMPT_EVAL_COUNT, EVAL_DURATION, EVAL_COUNT, PROMPT_EVAL_RATE, EVAL_RATE
...
class ScoreEstimator:
        ...
        def model_aggregator(df_group, k):
            return pd.Series({
                ...
                PROMPT_EVAL_RATE: 10**9 * df_group[PROMPT_EVAL_COUNT].mean() / df_group[PROMPT_EVAL_DURATION].mean(),
                EVAL_DURATION: df_group[EVAL_DURATION].mean() / 10**9,
                EVAL_RATE: 10**9 * df_group[EVAL_COUNT].mean() / df_group[EVAL_DURATION].mean(),
